...
installDirectory="/config/scripts/SQLBackups"
# ${PUID}=1234
# ${PGID}=1234
...
then
    echo "********** CLONING SQLBackups **********"
    error=$(git clone https://github.com/jsaddiction/SQLBackups.git ${installDirectory} 2>&1) || { echo "Failed to clone repo into ${installDirectory}. ERROR: ${error}" ; exit 1; }
else
    echo "********** UPDATING SQLBackups **********"
...
echo "********** Setting Permissions for SQLBackups **********"
...
echo "********** Installing SQLBackups Dependencies **********"
